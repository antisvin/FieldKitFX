...
void codec_start() {
	...
	if(HAL_I2SEx_TransmitReceive_DMA(
		&I2SHandle, reinterpret_cast<uint16_t *>(audio_output_buffer),
		reinterpret_cast<uint16_t *>(audio_input_buffer), AUDIO_IO_BUFFER_SIZE) != HAL_OK){
		__asm("bkpt");
	}
