...
void UI::render() {
	...
	case UI_tresholdcv_update:
		...
		if (rolloSelector.isSelected(ROLLO_SEQ)) {
			...
			rollo_env.processAdsrCv(
				rollodecks.cv_values[0] >> 2, rollodecks.cv_values[1] >> 2,
				rollodecks.cv_values[2] >> 2, rollodecks.cv_values[3] >> 2);
...
void UI::renderCalibration(){
	...
	current_ui_state_calibration = (UiStateCalibration)(
		((uint8_t)current_ui_state + 1) % NUMBEROFUISTATESCALIBRATION);
}
...
void UI::renderMagnitude(uint16_t magnitude){
	//fit it into the 11 leds <-- scaledMagnitude_max = 10
	volatile uint16_t scaledMagnitude = magnitude / 2979;
