...
void Frame::decode(float* stream) {
    for(uint8_t block_counter = 0; block_counter < BLOCKS_PER_FRAME; block_counter++) {
        uint16_t temp1 = ((samples[block_counter].nibbleByte & 0xf0) << 4) |
            samples[block_counter].lesser_samples[0];
        uint16_t temp2 = ((samples[block_counter].nibbleByte & 0x0f) << 8) |
            samples[block_counter].lesser_samples[1];
        // convert to float
