...
void ADSR::processThresholdPot(uint16_t CV1) {
    switch (mode) {
    case POT_ADSR:
        envelope_threshold = envelope_treshold_ma.process(
            inverselogLUT_10BitFloat[(uint16_t)ADSR_CV_RESOLUTION - CV1]);
        break;
    case CV_ADSR:
        envelope_threshold = envelope_treshold_ma.process(ADSR_CV_RESOLUTION - CV1);
        break;
    default:
        break;
    }
...
void ADSR::setLengthThreshold() {
    switch (mode) {
    case POT_ADSR:
        length = ADSR::mapTimeTarget(
            envelope_threshold, ADSR_MIN_OVERALL_TIME, ADSR_MAX_OVERALL_TIME);
        if (length < ADSR_MIN_OVERALL_TIME) {
            length = ADSR_MIN_OVERALL_TIME;
        };
        break;
    case CV_ADSR:
        threshold_cv = envelope_threshold;
        if (threshold_cv < 0) {
            threshold_cv = 0;
        };
        break;
...
void ADSR::processAdsrCv(uint16_t CVA, uint16_t CVD, uint16_t CVS, uint16_t CVR) {
    A_cv = envelope_potA_ma.process(inverselogLUT_10BitFloat[CVA]);
    if (A_cv < 0.0f) {
        A_cv = 0.0f;
    }

    D_cv = envelope_potD_ma.process(inverselogLUT_10BitFloat[CVD]);
    if (D_cv < 0.0f) {
        D_cv = 0.0f;
    }
    S_cv = (float)CVS;
    R_cv = envelope_potR_ma.process(inverselogLUT_10BitFloat[CVR]);
    if (R_cv < 0.0f) {
        R_cv = 0.0f;
    }
}
...
void ADSR::setA(void) {
    volatile float target =
        ADSR::mapTimeTarget((uint16_t)A_cv, ADSR_MIN_A_TIME, ADSR_MAX_A_TIME);
    A_inc = (ADSR_ACC_MAX_VAL / (target * ADSR_FS));
}

void ADSR::setD(void) {
    volatile float target =
        ADSR::mapTimeTarget((uint16_t)D_cv, ADSR_MIN_D_TIME, ADSR_MAX_D_TIME);
...
void ADSR::setR(void) {
    volatile float target =
        ADSR::mapTimeTarget((uint16_t)R_cv, ADSR_MIN_R_TIME, ADSR_MAX_R_TIME);
...
void ADSR::setROutput(void) {
    volatile float target =
        ADSR::mapTimeTarget(R_cv, ADSR_MIN_R_TIME, ADSR_MAX_R_TIME);
