...
void codec_ConfigGPIO(void) {
    ...
    GPIO_InitStruct.Pin =
        CODEC_I2S_SCK | CODEC_I2S_LRCLK | CODEC_I2S_SIN | CODEC_I2S_SOUT;
 ...
 * DMA Callbacks
 */
extern "C" void i2s_Xfr_halfCplt_RX(DMA_HandleTypeDef* hdma) {
    /*
    ...
    user_audio_in_buffer.reset();
}

extern "C" void i2s_Xfr_cplt_RX(DMA_HandleTypeDef* hdma) {
    /*
...
extern "C" void i2s_Xfr_halfCplt_TX(DMA_HandleTypeDef* hdma) {
    /*
    ...
    user_audio_out_buffer.reset();
}

extern "C" void i2s_Xfr_cplt_TX(DMA_HandleTypeDef* hdma) {
    /*
...
void codec_ConfigCodec_48K() {
    ...
    codec_writeRegister(CODEC_REG_RIGHT_LINE_IN,
        CODEC_INPUT_MUTE); // mute the right input channel
    ...
    codec_writeRegister(CODEC_REG_ANALOGUE_ROUTING,
        CODEC_ANALOGPATH_MUTEMIC | CODEC_ANALOGPATH_DACSEL);

    // Configure digital routing
    codec_writeRegister(CODEC_REG_DIGITAL_ROUTING, CODEC_DIGITALPATH_DEEMP_NONE);

    // Configure power management, let the oscillator powered
    codec_writeRegister(
        CODEC_REG_POWER_MANAGEMENT, CODEC_POWER_DOWN_MIC); // turn the mic off

    // Configure the format
    codec_writeRegister(CODEC_REG_DIGITAL_FORMAT,
        CODEC_FORMAT_MASK_16_BIT | CODEC_FORMAT_MASTER | CODEC_PROTOCOL_MASK_PHILIPS);
