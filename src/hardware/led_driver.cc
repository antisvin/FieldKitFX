...
void LedDriver::writeReg(uint8_t regAddr, uint8_t data, uint8_t autoInc) {
    ...
    // Send the data
    if(HAL_I2C_Master_Transmit(i2cController, (uint16_t)(i2cAddr << 1), temp, 2,
           I2C_TIMEOUT_VAL) != HAL_OK)
        __asm("bkpt");
}
 ...
 * The last parameter is used to set or not the AutoIncrement Flag (AIF)
 */
void LedDriver::write3Val(
    uint8_t startAddr, uint8_t data1, uint8_t data2, uint8_t data3) {
    // set the autoInc
    startAddr |= (1 << 7);
    ...
    // Send the data
    while(HAL_I2C_Master_Transmit(i2cController, (uint16_t)(i2cAddr << 1), temp,
              4, I2C_TIMEOUT_VAL) != HAL_OK) { }
